...
class CruiseWatchingTrayIconFrame(wx.Frame):
    ...
    def CheckBuild(self, evt):
    	...
    	if self.buildIsBrokenStatus:
            ...
            self.tbicon.SetIcon(icon, (self.configurationOptions.buildName or self.configurationOptions.cruiseUrl)+ ' build is broken')
        else:
            icon = wx.Icon('green.ico', wx.BITMAP_TYPE_ICO)
            self.tbicon.SetIcon(icon, (self.configurationOptions.buildName or self.configurationOptions.cruiseUrl)+ ' build is OK')
...
def main():
    try:
        opts, noArgsExpected = getopt(sys.argv[1:], "qhd:u:n:b:", [])
    except GetoptError:
        usage()
        sys.exit(2)
