...
class ColourMapper:
    ...
    def generate_colours(self):
        ...
        versions_by_author = {}
        map(lambda a: versions_by_author.setdefault(a, map(Source.get_version,
                                                           filter(lambda x: x.get_author() == a,
                                                                  self.sources.values()))), authors)
...
def makeHtml(args):
    ...
    for group in code_groups:
        ...
        result += '<tr><td class="revision" style="background-color: #%s">%s</td><td class="code">%s</td></tr>' % \
              (cm.get_version_colour(source.get_version()), source, string.join(body, '<br>\n'))
